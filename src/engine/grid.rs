...
impl Grid{
    pub fn new_default()->Self{
        ...
        *out.try_get_cell_mut((9,10)).expect("pos is in bounds") = GridCell::Head(PlayerId::new_o(), Direction::NegativeX);
        *out.try_get_cell_mut((11,10)).expect("pos is in bounds") = GridCell::Head(PlayerId::new_x(), Direction::PositiveX);
        out
    }
    pub fn next_grid(&self, player_a_choice: Direction, player_b_choice: Direction) -> NextFrameResult {
        ...
        let Some(next_a_pos) = next_a_pos else {return NextFrameResult::Winner { player_who_won: PlayerId::new_x() }};
        let Some(next_b_pos) = next_b_pos else {return NextFrameResult::Winner { player_who_won: PlayerId::new_o() }};
        ...
        if a_blocked {return NextFrameResult::Winner { player_who_won: PlayerId::new_x() }};
        if b_blocked {return NextFrameResult::Winner { player_who_won: PlayerId::new_o() }};
    ...
    pub fn get_cell_mut(&mut self, pos: impl Into<GridPosition>)->&mut GridCell{
        self.0.get_mut(pos.into().i() as usize).expect("position is in bounds")
    }
    ...
    pub fn get_cell(&self, pos: impl Into<GridPosition>)->&GridCell{
        self.0.get(pos.into().i() as usize).expect("position is in bounds")
    }
    ...
    pub fn head_positions_map(&self)->HashMap<PlayerId, GridPosition>{
        self.0
            ...
            .filter_map(|(pos, cell)|{
                ...
                Some((*player_id, GridPosition::new_from_usize(pos).expect("position is valid")))
            })
            .collect()
    }
